...
| [doc(sdk-node): encourage configuration of NodeSDK that does not result in default resource attributes being excluded by accident (#6857)](https://github.com/open-telemetry/opentelemetry-js/pull/6857) | trentm | trentm | ❌ | ✅ | 7m |
...
```text
PR #6857
llm: pr-conversation -> reviewer (The latest comment is from the author and shares a VSCode tooltip result as follow-up context, so the reviewer/maintainer needs to respond or decide next.)
