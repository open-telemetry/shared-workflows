...
| [chore(eslint): use canonical n/ prefix for eslint-plugin-n in flat config (#3581)](https://github.com/open-telemetry/opentelemetry-js-contrib/pull/3581) | bhuvan-somisetty |  | ✅ | ✅ | 23m |
| [fix(instrumentation-koa): use fallback name for anonymous middleware spans (#3582)](https://github.com/open-telemetry/opentelemetry-js-contrib/pull/3582) | bhuvan-somisetty |  | ✅ | ✅ | 23m |
| [fix(instrumentation-knex): use connectionSettings to support function-based connection configs (#3584)](https://github.com/open-telemetry/opentelemetry-js-contrib/pull/3584) | bhuvan-somisetty |  | ✅ | ✅ | 23m |
...
| [feat(several): only emit stable http, network and database attributes (#3585)](https://github.com/open-telemetry/opentelemetry-js-contrib/pull/3585) | maryliag | blumamir<br>jj22ee<br>martinkuba<br>maryliag<br>naseemkullah<br>neilime<br>onurtemizkan<br>pkanal<br>raphael-theriault-swi<br>seemk | ❌ | ✅ | 15m |
...
```text
PR #3585
llm: PRRT_kwDOD0IL486NW2FG -> author (The bot reviewer flagged a concrete mismatch in the metrics test and asked for a code/test update, so the PR author needs to act next.)
llm: PRRT_kwDOD0IL486NW2Fa -> author (The latest comment is a bot review noting a likely formatting/lint issue in the PR, so the author needs to fix the indentation and respond.)
llm: PRRT_kwDOD0IL486NW2Fu -> author (The latest comment is a review bot flagging README issues that still need to be fixed or addressed by the PR author.)

PR #3584
llm: pr-conversation -> reviewer (The author says they applied the suggested fix and added a test, so the ball is back with the reviewers to confirm or continue review.)
...
PR #3582
llm: pr-conversation -> reviewer (The latest comment is from the PR author asking for a review, so the next action is for a reviewer/maintainer to respond.)
