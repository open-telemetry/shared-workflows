...
| [Add Agent Framework reference scenario (#325)](https://github.com/open-telemetry/semantic-conventions-genai/pull/325) | eavanvalkenburg | lmolkova&nbsp;✅ | ✅ | ✅ | 18h |
...
| [Propose GenAI agent entity (#270)](https://github.com/open-telemetry/semantic-conventions-genai/pull/270) | aabmass | AgentGymLeader&nbsp;✔️<br>lmolkova&nbsp;✅<br>trask | ✅ | ✅ | 9h |
...
| [Migrate anthropic reference scenario to opentelemetry-util-genai (#324)](https://github.com/open-telemetry/semantic-conventions-genai/pull/324) | AgentGymLeader | lmolkova&nbsp;💬 | ✅ | ✅ | 2h |
...
| [Add gen_ai.agent.identity.id and gen_ai.agent.identity.name: agent identity distinct from the deployed resource (#350)](https://github.com/open-telemetry/semantic-conventions-genai/pull/350) | AraiYuno | 23h |
...
PR #324
llm: PRRT_kwDOSUeMrM6MqfKS -> author (A reviewer asked whether tests are needed for reference scenarios, so the author needs to जवाब/decide and likely add them or explain why not.)
llm: PRRT_kwDOSUeMrM6MqhDK -> author (A reviewer left a code suggestion and there is no author reply yet, so the author needs to apply or respond to it.)
llm: PRRT_kwDOSUeMrM6Mqhyu -> author (The latest comment is from a reviewer/approver asking for a code change, so the PR author needs to update the implementation.)
llm: PRRT_kwDOSUeMrM6MqiFi -> author (A reviewer asked to switch to an unreleased version and pointed to a specific upstream path, so the PR author needs to make that change or respond.)
llm: PRRT_kwDOSUeMrM6MqjEV -> author (A reviewer left a code suggestion and there’s no follow-up reply yet, so the PR author needs to act on it.)
llm: PRRT_kwDOSUeMrM6Mqjmt -> author (A reviewer asked to remove unrelated changes and narrow the PR scope, so the author needs to respond or update the branch.)
llm: PRRT_kwDOSUeMrM6Mqjv_ -> author (A reviewer left a code suggestion and there is no author reply yet, so the author needs to apply or respond to it.)
