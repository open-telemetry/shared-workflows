...
| PR | Author | Reviewers | CI | Conflicts | Age |
|---|---|---|:---:|:---:|:---:|
| [gen-ai: make input-messages BlobPart content optional and add stripped_reason (#144)](https://github.com/open-telemetry/semantic-conventions-genai/pull/144) | Mandark-droid | lmolkova<br>trask | ✅ | ❌ | 15d |
| [Update dependency google-adk to v2 (#328)](https://github.com/open-telemetry/semantic-conventions-genai/pull/328) | app/renovate |  | ❌ | ✅ | 6d |
| [Migrate anthropic reference scenario to opentelemetry-util-genai (#324)](https://github.com/open-telemetry/semantic-conventions-genai/pull/324) | AgentGymLeader |  | ✅ | ✅ | 6d |
...
| [gen-ai: model agent-to-agent handoff as execute_tool span (#98)](https://github.com/open-telemetry/semantic-conventions-genai/pull/98) | Krishnachaitanyakc | lmolkova<br>MikeGoldsmith&nbsp;✅<br>trask | ✅ | ❌ | 6h |
| [\[chore\] Add basic agents.md (#342)](https://github.com/open-telemetry/semantic-conventions-genai/pull/342) | lmolkova | trask&nbsp;💬⁠✅ | ✅ | ✅ | 2h |
| [Rename gen_ai.workflow.duration to gen_ai.invoke_workflow.duration (#341)](https://github.com/open-telemetry/semantic-conventions-genai/pull/341) | lmolkova |  | ✅ | ✅ | 1h |

## Waiting on authors

| PR | Author | Reviewers | CI | Conflicts | Age |
|---|---|---|:---:|:---:|:---:|
| [Add gen_ai.server.inter_token_latency metric (#164)](https://github.com/open-telemetry/semantic-conventions-genai/pull/164) | Jwrede | Cirilla-zmh<br>lmolkova&nbsp;💬<br>trask | ✅ | ❌ | 38d |
...
| [feat(gen-ai): add agent authorization observability attributes (#180) (#291)](https://github.com/open-telemetry/semantic-conventions-genai/pull/291) | thebenignhacker | lmolkova&nbsp;🔴 | ✅ | ❌ | 20h |
| [semconv for a2a protocol (#195)](https://github.com/open-telemetry/semantic-conventions-genai/pull/195) | eternalcuriouslearner | aabmass&nbsp;💬<br>JWinermaSplunk<br>pwkowalski&nbsp;💬⁠✔️<br>trask | ✅ | ✅ | 3h |
| [Introduce `gen_ai.invoke_agent.{inference,tool}_calls` (#336)](https://github.com/open-telemetry/semantic-conventions-genai/pull/336) | RKest | aabmass&nbsp;💬⁠✅<br>lmolkova&nbsp;💬⁠✅<br>trask&nbsp;💬 | ✅ | ❌ | 3h |
| [gen-ai: add run guardrail span and security finding (#262)](https://github.com/open-telemetry/semantic-conventions-genai/pull/262) | nagkumar91 | aabmass<br>habibam&nbsp;✔️<br>sjain700&nbsp;✔️<br>trask | ✅ | ❌ | 1h |
...
```text
PR #342
llm: PRRT_kwDOSUeMrM6L0npA -> reviewer (The author replied to the reviewer’s question and floated a possible CI-only check, so the discussion is back with the reviewer to react or decide whether that follow-up is needed.)
llm: PRRT_kwDOSUeMrM6MYVRk -> none (The author replied with the needed clarification and noted the existing file was merged elsewhere, so there’s no clear remaining follow-up in this thread.)
...
PR #291
llm: PRRT_kwDOSUeMrM6MHRum -> author (The latest comment is a reviewer asking a direct question about how the example should be populated, so the author needs to respond or clarify.)
llm: PRRT_kwDOSUeMrM6MZFo4 -> author (The bot raised a concrete formatting issue in `model/gen-ai/spans.yaml`, so the PR author needs to make the inline note a complete sentence.)
llm: PRRT_kwDOSUeMrM6MZFo- -> author (The latest comment is a bot review note requesting a documentation/formatting fix, so the PR author needs to update the file.)
llm: PRRT_kwDOSUeMrM6MZFpF -> author (A bot reviewer flagged a required formatting fix in `model/gen-ai/spans.yaml`, so the PR author needs to update the file.)
llm: PRRT_kwDOSUeMrM6MZFpL -> author (The latest comment is a review bot flagging an inconsistency in the scenario value, so the author needs to update the PR or respond.)
llm: pr-conversation -> author (The latest reviewer/approver comment asks the PR author to share details or document the convention, so the author needs to respond or adjust the PR.)
...
PR #195
llm: PRRT_kwDOSUeMrM6KRsU9 -> reviewer (The author asked whether they can resolve the thread, so the reviewer needs to confirm or respond.)
...
llm: PRRT_kwDOSUeMrM6MX-aI -> author (The last comment is a reviewer asking for clarification and raising concerns, so the PR author needs to जवाब/respond.)
...
PR #98
llm: pr-conversation -> reviewer (The author asked whether it can be added to the merge queue and if anything else needs addressing, so the next action is for a reviewer/maintainer to respond or merge.)
