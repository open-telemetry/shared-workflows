...
| [Add tools support in langchain (#37)](https://github.com/open-telemetry/opentelemetry-python-genai/pull/37) | wrisa | eternalcuriouslearner&nbsp;✅<br>lmolkova&nbsp;💬⁠✅ | ✅ | ✅ | 4m |
...
PR #37
llm: PRRT_kwDOSbspP86MWZOf -> author (A reviewer left a code suggestion and there is no author reply yet, so the PR author needs to act on it.)
llm: PRRT_kwDOSbspP86MWaXB -> author (The only comment is from an approver/reviewer and contains a suggestion, so the author needs to respond or apply the requested change.)
llm: PRRT_kwDOSbspP86MWasD -> author (A reviewer left a suggestion change with no author reply, so the PR author needs to act on it.)
