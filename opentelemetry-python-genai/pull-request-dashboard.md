...
| [feat: add instrumentation around openai responses stream method. (#131)](https://github.com/open-telemetry/opentelemetry-python-genai/pull/131) | eternalcuriouslearner | lmolkova&nbsp;✅ | ✅ | ✅ | 17h |
...
| [Move capture_message_content kwarg logic to genai-utils (#140)](https://github.com/open-telemetry/opentelemetry-python-genai/pull/140) | rads-1996 | lmolkova&nbsp;🔴 | ❌ | ✅ | 8d |
| [OpenAI agents: rewrite to util-genai (#90)](https://github.com/open-telemetry/opentelemetry-python-genai/pull/90) | lmolkova | rads-1996&nbsp;💬 | ❌ | ✅ | 21h |
| [Add Cohere instrumentation package scaffolding (#102)](https://github.com/open-telemetry/opentelemetry-python-genai/pull/102) | Nik-Reddy | eternalcuriouslearner&nbsp;✅<br>lmolkova&nbsp;💬<br>lzchen&nbsp;✅ | ✅ | ❌ | 10h |
| [\[`opentelemetry-instrumentation-google-genai`\] Instrument the embeddings API (#176)](https://github.com/open-telemetry/opentelemetry-python-genai/pull/176) | DylanRussell | lmolkova&nbsp;💬⁠✅ | ✅ | ✅ | 43m |
...
| [\[langchain\] Prototype collecting number of llm and tool calls per agent invocation (#173)](https://github.com/open-telemetry/opentelemetry-python-genai/pull/173) | lmolkova | 15h |
...
```text
PR #176
llm: PRRT_kwDOSbspP86MinVZ -> author (The latest comment is a bot review suggesting `yaml.safe_load` instead of `yaml.load`, so the PR author needs to update the code or जवाब back.)
llm: PRRT_kwDOSbspP86MinVo -> author (The bot reviewer flagged a bug in `OTelProviderSnapshot.restore()` and no one has responded yet, so the PR author needs to fix or address it.)
llm: PRRT_kwDOSbspP86MinV5 -> author (The latest comment is a bot review finding a likely bug in the instrumentation and asking for a code change, so the PR author needs to address it.)
llm: PRRT_kwDOSbspP86MinWO -> author (The bot reviewer pointed out a bug in `embeddings.py` where positional calls miss the model name, so the author needs to update the code or जवाब back with a fix.)
llm: PRRT_kwDOSbspP86MinWo -> author (The bot flagged a code issue that still needs a fix; no author reply has addressed it yet.)
llm: PRRT_kwDOSbspP86MjU2b -> author (A reviewer suggested adding conformance tests; even though it’s not blocking, the author still needs to respond or decide whether to implement it.)
...
PR #124
llm: PRRT_kwDOSbspP86K8dTl -> reviewer (The author replied with an issue link to address the reviewer’s request, so the ball is back with the reviewer to accept that follow-up or continue the discussion.)
