...
| [\[jdbc\] Capture custom object types in prepared statement parameter instrumentation (#19093)](https://github.com/open-telemetry/opentelemetry-java-instrumentation/pull/19093) | CodingFabian | laurit | ✅ | ✅ | 22h |
| [Remove SpEL from SpringConfigProperties.getMap() (#19113)](https://github.com/open-telemetry/opentelemetry-java-instrumentation/pull/19113) | zeitlinger |  | ⏳ | ✅ | 45m |
...
| [switch non-inlined instrumentation by default + update doc (#19076)](https://github.com/open-telemetry/opentelemetry-java-instrumentation/pull/19076) | SylvainJuge | laurit<br>SylvainJuge | ✅ | ✅ | 25m |
...
| [Rename setCaptured* to setCapture* to have a single convention (#17154)](https://github.com/open-telemetry/opentelemetry-java-instrumentation/pull/17154) | trask | 11h |
...
llm: PRRT_kwDODJKVX86Nndv9 -> author (The latest comment is from a reviewer asking for a naming decision on the docs, so the author needs to जवाब/adjust the PR.)
...
PR #19076
llm: PRRT_kwDODJKVX86NnsVg -> author (The latest comment is a bot review finding that asks to restore the null check because the current change can cause NPEs, so the PR author needs to fix the code.)
llm: PRRT_kwDODJKVX86NnsWB -> author (The latest comment is a review note asking for an additional test/fixture to cover the new default-derivation branch, so the PR author needs to make the change.)
...
llm: PRRT_kwDODJKVX86NkAf- -> author (A reviewer asked a direct question about the change and no author reply is present, so the author needs to respond.)
...
llm: PRRT_kwDODJKVX86NkFhB -> author (A reviewer suggested changing the cache implementation to `ClassValue`, so the author needs to respond or update the code.)
llm: PRRT_kwDODJKVX86NkGaW -> author (A reviewer asked whether the hierarchy check is necessary; the author needs to जवाब/justify or adjust the code.)
llm: PRRT_kwDODJKVX86NkM2A -> author (A reviewer asked why reflection is needed for a public method, so the author needs to जवाब/adjust the code.)
llm: PRRT_kwDODJKVX86Nkj0y -> author (A reviewer raised a concrete design concern and suggested changing how cluster id is applied; the author needs to respond or update the code.)
llm: PRRT_kwDODJKVX86NkoeA -> author (A reviewer flagged that non-spec attributes should not be emitted by default, so the author needs to update the code or respond to the convention.)
