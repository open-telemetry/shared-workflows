...
| [Align DynamoDB batch telemetry with database batch semantics (#19034)](https://github.com/open-telemetry/opentelemetry-java-instrumentation/pull/19034) | trask | laurit&nbsp;✅ | ✅ | ✅ | 18h |
| [feat: add reactive command support for lettuce-4.0 instrumentation (#19071)](https://github.com/open-telemetry/opentelemetry-java-instrumentation/pull/19071) | YaoYingLong | trask&nbsp;✅ | ✅ | ✅ | 17h |
...
| [fix(lettuce): set span status for Redis command errors in \[5.1.0, 6.0.0) (#19075)](https://github.com/open-telemetry/opentelemetry-java-instrumentation/pull/19075) | YaoYingLong |  | ✅ | ✅ | 8h |
...
| [Fix ambiguous IPv6 address in db.connection_string for MySQL/MariaDB (#19078)](https://github.com/open-telemetry/opentelemetry-java-instrumentation/pull/19078) | bhuvan-somisetty |  | ✅ | ✅ | 5h |
| [Add Cassandra JMX metrics target system (#19080)](https://github.com/open-telemetry/opentelemetry-java-instrumentation/pull/19080) | jkoronaAtCisco | trask | ❌ | ✅ | 2h |
...
| [switch non-inlined instrumentation by default + update doc (#19076)](https://github.com/open-telemetry/opentelemetry-java-instrumentation/pull/19076) | SylvainJuge | 6h |
| [Gate process command attributes under v3 preview (#19082)](https://github.com/open-telemetry/opentelemetry-java-instrumentation/pull/19082) | trask | 34m |
...
PR #19054
llm: PRRT_kwDODJKVX86Lq7Rc -> reviewer (The latest comment is from the PR author showing the copied change/diff, so the ball is back with a reviewer to review or respond.)
llm: PRRT_kwDODJKVX86Lq7Rm -> reviewer (The latest comment is from the PR author and presents a proposed diff/update, so the ball is back with the reviewer to review or respond.)
llm: PRRT_kwDODJKVX86Lq7Rq -> reviewer (The only comment is from the PR author and appears to be a proposed code change / reply, so the ball is with a reviewer to review or respond.)
llm: PRRT_kwDODJKVX86Lq7Rt -> reviewer (The latest comment is from the PR author and presents a code change/diff, so the ball is back with reviewers to review or approve it.)
llm: PRRT_kwDODJKVX86Lq7Rz -> reviewer (The author’s last comment is a proposed code change/diff, so the ball is back with the reviewer to review or respond.)
llm: PRRT_kwDODJKVX86Lq7R1 -> reviewer (The latest comment is from the PR author and appears to be a reply/update, so the reviewer/maintainer is next to respond or review.)
llm: PRRT_kwDODJKVX86Lq7R5 -> reviewer (The latest comment is from the author and appears to propose or explain a change, so the ball is with a reviewer/maintainer to respond or review next.)
llm: PRRT_kwDODJKVX86Lq7R6 -> reviewer (The latest comment is from the author and appears to be a reply/update, so the reviewer/maintainer has the next action if anything remains to be checked.)
llm: PRRT_kwDODJKVX86Lq7R7 -> reviewer (The latest comment is from the author and appears to be a proposed code change/diff, so the ball is back with a reviewer to review or respond.)
llm: PRRT_kwDODJKVX86Lq7SC -> reviewer (The latest comment is from the PR author and appears to be a code change/update, so the ball is with the reviewer to review or respond.)
llm: PRRT_kwDODJKVX86Lq7SF -> reviewer (The latest comment is from the PR author and provides a code/diff explanation, so the ball is with a reviewer to respond or approve.)
llm: PRRT_kwDODJKVX86Lq7SK -> reviewer (The latest comment is from the PR author and presents a code change/diff, so the ball is with a reviewer to review or respond.)
llm: PRRT_kwDODJKVX86Lq7SO -> reviewer (The latest comment is from the PR author and presents a proposed code change, so the reviewer/maintainer is the next to review or respond.)
llm: PRRT_kwDODJKVX86Lq7SW -> reviewer (The only comment is from the PR author and reads like a proposed code change, so the ball is with a reviewer to respond or review it.)
llm: PRRT_kwDODJKVX86Lq7Sa -> reviewer (The latest comment is from the PR author and appears to present the proposed code change, so the reviewer/maintainer needs to respond or review next.)
llm: PRRT_kwDODJKVX86Lq7Sd -> reviewer (The latest comment is from the PR author and appears to be a code change/context note, so the ball is back with the reviewer to respond or review.)
