...
# older state is ignored and regenerated by the next dashboard run. If that
# run is triggered for one PR (--pr-number), the missing cache forces a full
# rebuild of all open non-draft PRs, not just that one PR.
STATE_VERSION = 3
