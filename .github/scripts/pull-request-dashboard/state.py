...
# State files are disposable workflow caches, not durable user data. When a
# stored state shape or cached routing meaning changes, bump this version so
# older state is ignored and regenerated by the next non-PR dashboard run.
# Targeted PR runs skip until initial dashboard state has been built.
STATE_VERSION = 3
...
def empty_state() -> dict[str, Any]:
    return {"version": STATE_VERSION, "prs": {}}


def empty_backfill_state() -> dict[str, Any]:
    return {"version": STATE_VERSION, "cursor": {}}
...
def load_backfill_state() -> dict[str, Any]:
    state = load_state_file(backfill_state_path())
    if state is None:
...
def save_backfill_state(state: dict[str, Any]) -> None:
    stored = {k: v for k, v in state.items() if k != "prs" and not k.startswith("_")}
    stored["version"] = STATE_VERSION
    stored.setdefault("cursor", {})
    path = backfill_state_path()
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(json.dumps(stored, sort_keys=True, indent=2), encoding="utf-8")
...
def _load_notification_state_file() -> dict[str, Any] | None:
...
def load_notifications() -> dict[str, Any] | None:
    state = _load_notification_state_file()
    if state is None:
        return None
    return state["prs"]
