...
class DeliveryTest(unittest.TestCase):
    ...
    def test_targeted_missing_pr_runs_status_cleanup_after_reply_failure(self) -> None:
        def fail_command_reply(
            _repo: str,
            received_failed_prs: set[int],
            _pr_number: int,
        ) -> list[str]:
            received_failed_prs.add(7)
        ...
        with (
            ...
            patch.object(
                delivery,
                "deliver_dashboard_command_replies",
                side_effect=fail_command_reply,
            ),
            patch.object(delivery, "deliver_prepared_author_nudges", return_value=[]),
            patch.object(
                delivery,
