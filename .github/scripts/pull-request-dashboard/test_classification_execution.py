...
class ClassificationFacadeCharacterizationTest(unittest.TestCase):
    def _stores(self) -> tuple[
        ...
        direct_temp = tempfile.TemporaryDirectory()
        facade_temp = tempfile.TemporaryDirectory()
        self.addCleanup(direct_temp.cleanup)
        self.addCleanup(facade_temp.cleanup)
        return (
            direct_temp,
            FileClassificationCacheStore(Path(direct_temp.name)),
            facade_temp,
            FileClassificationCacheStore(Path(facade_temp.name)),
        )
    ...
    def _run_pair(
        ...
        *,
        ...
        direct_runner = FakeModelRunner(
            responses,
            responder=responder if not responses else None,
        )
        facade_runner = FakeModelRunner(
            responses,
            responder=responder if not responses else None,
        )
        options = service_options or {}
        direct_service = ClassificationService(
            direct_runner,
            direct_store,
            **options,
        )
        facade_service = ClassificationService(
            facade_runner,
            facade_store,
            **options,
        )
        ...
        direct_results = [
            direct_service.classify(request)
            for _ in range(runs)
        ]
        with patch.object(
            classification,
            ...
            facade_results = [
                classification.classify_discussion_domains(
                    request.pr_number,
                    list(review_threads),
                    list(top_level_items),
                    list(author_comments),
                    request.model,
                )
                for _ in range(runs)
            ]
        self.assertEqual(facade_results, direct_results)
        self.assertEqual(facade_runner.requests, direct_runner.requests)
        self.assertEqual(
            (facade_store.directory / "123.json").read_text(encoding="utf-8"),
            (direct_store.directory / "123.json").read_text(encoding="utf-8"),
        )
        return (
            direct_results,
            direct_runner,
    ...
    def test_cache_miss_hit_and_batched_verdicts_match(self) -> None:
        records = tuple(
            discussion_record(f"feedback-{index}")
            for index in range(11)
        )

        _results, runner, _store, *_rest = self._run_pair(
            top_level_items=records,
            runs=2,
        )

        self.assertEqual(len(runner.requests), 2)
        self.assertEqual(
            [len(prompt_items(request)) for request in runner.requests],
            [10, 1],
        )

    def test_author_comment_budget_matches(self) -> None:
        records = tuple(
            discussion_record(
                f"reply-{index}",
                DiscussionKind.TOP_LEVEL_AUTHOR_REPLY,
        ...
        results, runner, _store, *_rest = self._run_pair(
            author_comments=records,
            service_options={
                "batch_size": 1,
                "max_author_comment_model_calls_per_pr": 2,
            },
        ...
        self.assertEqual(
            [
                result.deferred
                for result in results[0].top_level_author_comments
            ],
            [False, False, True],
        )

    def test_failure_timeout_and_malformed_response_match(self) -> None:
        record = discussion_record("feedback")
        responses = (
            RawModelResponse(5, "failed response", "failed stderr"),
            subprocess.TimeoutExpired(
                "copilot",
                LLM_DISCUSSION_TIMEOUT_SECONDS,
                output="partial",
            ...
            RawModelResponse(0, "not json", "bad response"),
        )
        for name, response in zip(
            ...
            with self.subTest(name=name):
                self._run_pair(
                    top_level_items=(record,),
                    responses=(response,),
                )

    def test_cache_pruning_matches(self) -> None:
        (
            _direct_temp,
            direct_store,
            _facade_temp,
        ...
        for store in (direct_store, facade_store):
            store.write(1, {"one": {}})
            store.write(2, {"two": {}})
            (store.directory / "notes.json").write_text(
                "{}",
                encoding="utf-8",
            )

        direct_store.prune({2})
        with patch.object(
            classification,
            "DEFAULT_CLASSIFICATION_CACHE_STORE",
            facade_store,
        ):
            classification.prune_classification_cache({2})

        self.assertEqual(
            sorted(path.name for path in direct_store.directory.iterdir()),
            sorted(path.name for path in facade_store.directory.iterdir()),
        )
        self.assertEqual(
            (direct_store.directory / "2.json").read_text(encoding="utf-8"),
            (facade_store.directory / "2.json").read_text(encoding="utf-8"),
        )
