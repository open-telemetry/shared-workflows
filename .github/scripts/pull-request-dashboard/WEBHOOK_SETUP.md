...
| ---------- | ------ | ---------------- |
| Checks | Read | Required to read check data for dashboard rows. |
| Commit statuses | Read | Required to read status contexts, such as EasyCLA, in the check rollup. Those checks are reported as statuses rather than check runs. |
...
Do not subscribe to Check run, Check suite, or Status. Those events arrive about
ten times per push, report no pull request number when the head branch lives in
a fork, and the dashboard's own workflow runs produced check suites that
dispatched further runs. The scheduled poll in section 6 covers CI status
instead.
...
## 6. Scheduled check poll

`dashboard-check-poll.mjs` is a Netlify scheduled function that dispatches
`pull-request-dashboard-check-poll.yml` every ten minutes. That workflow asks
each configured repository, in one GraphQL query, for every open pull request's
head commit and status check rollup state, compares the answer against the
baseline stored on the `otelbot/pull-request-dashboard-poll-state` branch, and
dispatches a targeted refresh only for pull requests whose head or rollup
changed.

GitHub's own `schedule` trigger is best effort and does not hold a cadence below
an hour, so the workflow keeps an hourly `schedule` only as a backstop for a
missed Netlify run.

The poll needs no Netlify configuration beyond the dispatcher credentials in
section 4. It reads the target repositories with the target repository app
instead, whose private key stays inside GitHub Actions.

The first poll of a repository records a baseline without dispatching anything,
because every open pull request would otherwise look changed.
